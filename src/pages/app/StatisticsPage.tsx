...
export function StatisticsPage() {
  ...
  const maxOtherMonth = Math.max(1, ...otherMonthStats.map((b) => b.value));
    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-soft">
          Jen odlétané akce — koncepty, potvrzené a zrušené akce se do čísel nepočítají.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <>
          <section className="flex flex-col gap-6">
            <h2 className="font-display text-xl font-bold">Srnčata</h2>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-2xl border border-line bg-bg-raised p-5">
                <h3 className="font-display text-lg font-bold">Jakým stylem</h3>
                {summary.rescued === 0 ? (
                  ...
                  <>
                    <div className="mt-4 flex h-4 w-full overflow-hidden rounded-full">
                      <div
                        style={{
                          width: `${(summary.caught / summary.rescued) * 100}%`,
                          background: "var(--meadow)",
                        }}
                      />
                      <div
                        style={{
                          width: `${(summary.chased / summary.rescued) * 100}%`,
              ...
              <div className="rounded-2xl border border-line bg-bg-raised p-5">
                <h3 className="font-display text-lg font-bold">Podle typu porostu</h3>
              ...
              <div className="rounded-2xl border border-line bg-bg-raised p-5 lg:col-span-2">
                <h3 className="font-display text-lg font-bold">Podle pilota</h3>
                <div className="mt-4 flex flex-col gap-3">
                  {pilotStats.length === 0 ? (
                    <p className="text-sm text-ink-soft">Zatím žádná data.</p>
                  ) : (
              ...
              <div className="rounded-2xl border border-line bg-bg-raised p-5 lg:col-span-2">
                <h3 className="font-display text-lg font-bold">Podle měsíce</h3>
                <div className="mt-4 flex flex-col gap-3">
                  {monthStats.length === 0 ? (
                    <p className="text-sm text-ink-soft">Zatím žádná data.</p>
                  ) : (
          ...
          <section className="flex flex-col gap-6">
            <div>
              <h2 className="font-display text-xl font-bold">Jiné akce</h2>
              <p className="mt-1 text-sm text-ink-soft">
                Přednášky pro školy a jiné výjezdy, podle kalendářního měsíce.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-line bg-bg-raised p-5">
              <h3 className="font-display text-lg font-bold">Podle měsíce</h3>
              <div className="mt-4 flex flex-col gap-3">
                {otherMonthStats.length === 0 ? (
                  <p className="text-sm text-ink-soft">Zatím žádná data.</p>
                ) : (
                  otherMonthStats.map((b) => (
                    <BarRow key={b.label} label={b.label} value={b.value} max={maxOtherMonth} color="var(--meadow)" />
                  ))
                )}
              </div>
            </div>
